...
## 16. Parallel Truth Sources Drift Fast

**Symptom**: `feature_list.json`, `progress.md`, and active specs all disagree about what is in progress.

**Cause**: New harness files were added on top of an existing workflow instead of adapting the existing state source.

**Fix**: Pick one primary control plane. If the repo is already spec-driven, keep the active spec as the main state carrier unless the user explicitly wants a migration.

---

## 17. Mandatory Bootstrap Scripts Create Fake Rigor

**Symptom**: Repo has `init.sh`, but nobody trusts it, and agents bypass it immediately.

**Cause**: A full bootstrap script was copied in because the template had one, not because the repo needed one.

**Fix**: Define readiness gates that match the repo. Sometimes `init.sh` is right. Sometimes a small readiness checklist or a single smoke command is enough.
...
- [Repo Adaptation Pattern](repo-adaptation-pattern.md) — Gotcha #16
- [Spec-Driven Harness Pattern](spec-driven-harness-pattern.md) — Gotcha #16
